...
const filteredScores = computed(() => {
  if (activeTab.value === 'Overall') {
    // Sort all scores by total_score DESC
...
const getRankColor = (index) => {
  if (index === 0) return 'text-signal-gold'; // Gold
  if (index === 1) return 'text-pixel-lavender'; // Silver
  if (index === 2) return 'text-byte-coral'; // Bronze
  ...
  <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 text-pixel-plum">
    ...
    <div v-if="topThree.length > 0" class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
      ...
      >
        <!-- Rank Icon -->
        <div 
          class="absolute -top-5 w-12 h-12 rounded-lg bg-white border-2 flex items-center justify-center font-black text-xl shadow-pixel-soft"
          :class="[
            index === 0 ? 'border-signal-gold text-signal-gold' : 
        ...
        <div class="px-4 py-1.5 bg-pixel-plum text-white rounded font-mono font-black text-sm">
    ...
    <div class="pixel-card">
        ...
        <table class="w-full text-left">
          ...
          <tbody class="divide-y divide-pixel-plum/5">
            <tr v-for="(player, index) in (topThree.length > 0 ? otherScores : filteredScores)" :key="player.score_id" class="group hover:bg-pixel-violet/5 transition-colors">
              ...
              <td class="py-5 px-4">
                <span class="text-sm font-black text-pixel-plum">{{ player.username }}</span>
              </td>
              <td class="py-5 px-4">
                <span class="text-[9px] font-black px-2 py-1 bg-pixel-plum/5 text-pixel-plum/60 rounded uppercase tracking-widest">{{ player.map_name }}</span>
              </td>
              <td class="py-5 px-4 text-right">
                <span class="text-sm font-black font-mono text-pixel-violet">{{ player.total_score }}</span>
              </td>
              <td class="py-5 px-4 text-right text-[10px] font-bold text-pixel-plum/40 uppercase">
      ...
      <!-- Empty State -->
      <div v-if="filteredScores.length === 0" class="py-20 text-center">
        <p class="text-pixel-plum/40 font-black uppercase tracking-widest">No rankings recorded for this sector</p>
      </div>
    </div>
  </div>
</template>
