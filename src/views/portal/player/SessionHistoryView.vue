  ...
  <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 text-pixel-plum">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
      ...
      <div class="flex gap-4">
        <div class="px-4 py-2 bg-white/50 border border-pixel-plum/10 rounded-lg shadow-sm">
          <p class="text-9 uppercase font-black text-pixel-plum/40 tracking-widest mb-1">Cleared</p>
          <p class="text-xl font-black text-pixel-moss font-mono">{{ stats.cleared }}</p>
        </div>
        <div class="px-4 py-2 bg-white/50 border border-pixel-plum/10 rounded-lg shadow-sm">
          <p class="text-9 uppercase font-black text-pixel-plum/40 tracking-widest mb-1">Failed</p>
          <p class="text-xl font-black text-byte-coral font-mono">{{ stats.failed }}</p>
        </div>
        <div class="px-4 py-2 bg-white/50 border border-pixel-plum/10 rounded-lg shadow-sm">
          <p class="text-9 uppercase font-black text-pixel-plum/40 tracking-widest mb-1">Total Yield</p>
    ...
    <div class="pixel-card">
      ...
      <div v-if="isLoading" class="py-20 text-center flex flex-col items-center gap-4">
        ...
        <p class="text-10 text-pixel-plum/40 font-black uppercase tracking-widest animate-pulse">Syncing Personnel Records...</p>
      </div>
        ...
        <table class="w-full text-left" v-if="sortedSessions.length > 0">
          <thead>
            <tr class="border-b border-pixel-plum/10">
              <th class="pb-4 text-10 uppercase text-pixel-plum/50 font-black tracking-widest px-4">Mission Target</th>
              <th class="pb-4 text-10 uppercase text-pixel-plum/50 font-black tracking-widest px-4">Deployment Status</th>
              <th class="pb-4 text-10 uppercase text-pixel-plum/50 font-black tracking-widest px-4 text-right">Credit Delta</th>
              <th class="pb-4 text-10 uppercase text-pixel-plum/50 font-black tracking-widest px-4 text-right">Duration</th>
              <th class="pb-4 text-10 uppercase text-pixel-plum/50 font-black tracking-widest px-4 text-right">Timestamp</th>
            </tr>
          </thead>
            ...
            <tr v-for="session in sortedSessions" :key="session.session_id" class="group hover:bg-pixel-violet/5 transition-colors">
              <td class="py-5 px-4">
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex flex-col">
                    ...
                    <span class="text-9 text-pixel-plum/40 font-bold uppercase tracking-wider">Sector ID: 00{{ session.session_id.slice(0, 4) }}</span>
                  </div>
                </div>
              </td>
              <td class="py-5 px-4">
                <span class="text-9 px-3 py-1 border-2 rounded font-black tracking-widest uppercase" 
              ...
              <td class="py-5 px-4 text-right">
                <div class="flex flex-col items-end">
                  ...
                  <span class="text-9 text-byte-coral/70 font-bold font-mono" v-if="session.credits_lost > 0">-{{ session.credits_lost }}</span>
              ...
              <td class="py-5 px-4 text-right">
                <div class="flex flex-col items-end">
                  ...
                  <span class="text-10 text-pixel-plum/40 font-black font-mono">{{ formatTime(session.played_at) }}</span>
                </div>
              </td>
            </tr>
          </tbody>
